...
namespace SRPG.Data
{
    ...
    public sealed class BattleAudioBank : ScriptableObject
    {
        ...
        [Header("거리 감쇠 — 전장 한 변에 대한 비율")]

        /// <summary>
        /// 최대 음량으로 들리는 반경입니다. 전장 한 변에 대한 비율입니다.
        ///
        /// <b>화면에 보이는 싸움이 이 안에 들어와야 합니다.</b>
        /// 이 값이 작으면 보이는 전투가 이미 감쇠 구간이라 소리가 거의 들리지 않습니다.
        ///
        /// <b>거리를 재는 기준은 카메라가 아니라 초점입니다.</b>
        /// 리그가 듣는 귀를 초점에 세워 둡니다 — 카메라는 직교 투영이라 초점에서 한참 물러나 있고,
        /// 그 거리를 소리에 끌고 오면 전장 전체가 무음 반경 밖으로 나갑니다.
        /// 여기 적는 비율은 <b>전장에서의 실제 거리</b>로 읽힌다는 뜻입니다.
        /// </summary>
        [Range(0.1f, 2f)]
        [Tooltip("최대 음량으로 들리는 반경입니다. 전장 한 변에 대한 비율입니다.\n" +
                 "거리는 카메라가 아니라 화면의 초점에서 잽니다.")]
        public float SfxFullVolumeRatio = 0.3f;

        /// <summary>
        /// 이 거리를 넘으면 들리지 않습니다. 전장 한 변에 대한 비율입니다.
        ///
        /// 전장 한 변보다 넉넉히 잡습니다. 화면 밖의 싸움도 <b>희미하게는</b> 들려야
        /// 어디서 전선이 무너지는지 눈을 돌리기 전에 알 수 있습니다.
        /// </summary>
        [Range(0.5f, 6f)]
        [Tooltip("들리지 않게 되는 거리입니다. 전장 한 변에 대한 비율입니다.\n" +
                 "화면 밖의 싸움도 희미하게는 들리도록 한 변보다 넉넉히 잡습니다.")]
        public float SfxSilenceRatio = 1.5f;
