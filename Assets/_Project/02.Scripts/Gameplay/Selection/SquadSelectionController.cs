...
namespace SRPG.Gameplay.Selection
{
    ...
    public sealed class SquadSelectionController : MonoBehaviour
    {
        ...
        /// <summary>
        /// 이번 프레임의 입력을 읽고 표시와 시간 배율을 갱신합니다.
        ///
        /// <b>왜 진입점이 부르는가</b>
        ///
        /// 여기서 내린 명령(<see cref="Squad.IssueMoveOrder"/>)은 <b>같은 프레임에</b>
        /// 앵커 전진으로 이어져야 합니다. <c>Update</c> 로 두면 분대보다 나중에 돌 수 있고,
        /// 그러면 클릭이 언제나 한 프레임 늦게 듣습니다. 눈에 띄지는 않지만
        /// 프레임마다 달라지는 종류의 어긋남이라 재현도 되지 않습니다.
        /// 지금은 조립 계층의 <c>BattleEntryPoint</c> 가 <b>가장 먼저</b> 부릅니다.
        ///
        /// <b>지난 시간을 받지 않습니다.</b> 여기서 시간을 재는 것이 없기 때문입니다 —
        /// 입력은 프레임 단위의 사건이고, 슬로우모션 전환을 실제로 보간하는 것은
        /// <see cref="TacticalTimeController"/> 입니다. 쓰지 않을 인자를 받아 두면
        /// 언젠가 여기서 시간을 재기 시작하고, 그 값이 <b>스케일된 시간</b>이라
        /// 시간이 느려질수록 입력이 굼떠지는 결함이 조용히 생깁니다.
        /// </summary>
        public void Tick()
        {
