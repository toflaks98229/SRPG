...
namespace SRPG.Gameplay.Squads
{
    ...
    public sealed class SquadMembers
    {
        ...
        /// <b>왜 분대가 병사를 돌리는가</b>
        ///
        /// 예전에는 병사마다 <c>Update</c> 가 있었습니다. 유니티는 그 메시지를 스크립트마다
        /// 따로 찾아 불러야 하고, 그 비용은 <b>안에서 무엇을 하든 똑같이</b> 듭니다.
        /// 병사 200명이면 관리되는 호출이 200번이고, 여기로 옮기면 분대 수만큼인 10번 안팎이 됩니다.
        /// 안에서 도는 일은 그대로이므로 이것은 순수한 호출 오버헤드의 제거입니다.
        ///
        /// <b>그리고 순서가 생깁니다.</b> 유니티의 <c>Update</c> 순서는 정해져 있지 않아서,
        /// 병사가 이번 프레임의 슬롯을 받기 <b>전에</b> 그 자리로 걸어갈 수도 있었습니다.
        /// 한 프레임 늦은 자리를 향하는 것이라 눈에 띄지는 않지만, 고칠 수도 없는 종류의 어긋남이었습니다.
        /// 지금은 분대가 자리를 정한 <b>다음에</b> 병사가 움직입니다.
        ///
        /// <b>순회 중 죽는 병사</b>
        ///
        /// 한 병사가 다른 병사를 죽일 수 있습니다. 그래서 목록을 만지지 않고 인덱스로 훑되,
        /// 이미 쓰러진 병사는 건너뜁니다. 명부에서 빼는 것은 다음 프레임의
        /// <see cref="PruneDead"/> 가 합니다 — 순회 도중 목록을 줄이면 누군가의 차례가 통째로 날아갑니다.
        /// </summary>
